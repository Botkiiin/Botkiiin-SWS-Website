...
import { useCallback, useEffect, useState } from 'react';
 ...
 function LanguageDetectionAlert() {
  ...
  const { showLanguageAlert, setShowLanguageAlert, dismissLanguageAlert, getLangAlertDismissed } =
    useLocaleStore();
  ...
  useEffect(() => {
    const detectedLang = navigator.language; // Get full language code, e.g.,
  ...
  return (
    <div
      className={cn(
        ...
        isVisible ? 'translate-x-0 translate-y-0 opacity-100' : 'translate-x-full opacity-0'
      )}
      role="banner"
      aria-live="polite"
      ...
      <div className="bg-background/95 backdrop-blur-md border border-border rounded-xl shadow-lg p-4 relative">
        ...
        <div className="pr-8">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-medium text-sm text-foreground">{alertMessages.title}</h3>
          </div>
...
export default LanguageDetectionAlert
